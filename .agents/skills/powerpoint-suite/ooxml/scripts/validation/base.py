...
class BaseSchemaValidator:
        ...
        for xml_file in self.xml_files:
            try:
                ...
                for attr_val in [v for k, v in root.attrib.items() if k.endswith('Ignorable')]:
                    ...
                    errors.extend(f"  {xml_file.relative_to(self.unpacked_dir)}: "
    ...
    def validate_against_xsd(self):
        ...
        # Get errors from original file
        original_errors = self._get_original_errors()
        ...
        for xml_file in self.xml_files:
            ...
            original_for_file = original_errors.get(relative_path, set())
            new_file_errors = current_errors - original_for_file

            if new_file_errors:
                # These are genuinely new errors
                new_errors.append(
                ...
                for error in list(new_file_errors)[:3]:  # Show first 3 errors
                    ...
                    )  # Truncate long messages to 250 chars
            elif current_errors:
                # All errors existed in original
                original_error_count += 1
                valid_count += 1  # Count as valid since no NEW errors
    ...
    def _get_original_errors(self):
        """Get XSD validation errors from the original file."""
        import tempfile
        import zipfile
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Validate each file in original
            for xml_file in temp_path.rglob("*.xml"):
                is_valid, errors = self._validate_single_file_xsd(xml_file, temp_path)
                if errors:  # Only store if there are errors
                    relative_path = str(xml_file.relative_to(temp_path))
                    original_errors[relative_path] = errors
